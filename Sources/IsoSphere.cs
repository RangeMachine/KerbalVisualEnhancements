...
namespace Geometry
{
    ...
    public static class IsoSphere
    {
        ...
        public static void Create(GameObject gameObject, float height, CelestialBody celestialBody)
        {
            ...
            ConfigNode kveConfig = GameDatabase.Instance.GetConfigNodes("KERBAL_VISUAL_ENHANCEMENTS")[0];

            int recursionLevel = 4;
            int.TryParse(kveConfig.GetValue("recursionLevel"), out recursionLevel);

            if (celestialBody != null)
            {
                recursionLevel = 6;
                int.TryParse(kveConfig.GetValue("recursionLevelBody"), out recursionLevel);
            }
